...
//        console.info(_t, _t.children(argfind).length, argfind);
    ...
    $.fn.zoomvideoplayer = function(o) {
        ...
        var download = $('#file_download').val();
        ...
        var defaults = {
            ...
            , cue: "on"
            ,autoplay: "off"
            ,embedded: "off" // -- reveals if embedded or not
...
//        console.info(this, o);
        ...
        this.each( function(){
            ...
            function init(){
                ...
                if(embed_code==''){
                    if(cthis.find('.feed-embed').length){
                        embed_code = cthis.find('.feed-embed').eq(0).html();
                        cthis.find('.feed-embed').remove();
                    }

                    // console.info('embed_code', embed_code);
                    embed_code = embed_code.replace('{{source}}', cthis.attr('data-source'));
                    embed_code = embed_code.replace('{{cover}}', cthis.attr('data-source'));
                    embed_code = embed_code.replace('onload="', 'onload=\'');
                    embed_code = embed_code.replace('"></i', '\'></i');
                    // console.info('embed_code', embed_code);
                }
...
//                console.info(o.design_skin, str_controls);
                ...
                if(cthis.find('.embed-btn').length>0){
                    ...
                    if(embed_code==''){
                        ...
                        _embedBtn.find('textarea').html(embed_code);
                    }
                }
...
//                    console.info(cthis, cthis.attr('data-arg_logoimg'));
                ...
                //console.info(cthis, dataSrc);

                if(o.type=='detect' && dataSrc.length>3 && ( dataSrc.indexOf('.png') == dataSrc.length-4 || dataSrc.indexOf('.jpg') == dataSrc.length-4 || dataSrc.indexOf('.gif') == dataSrc.length-4 )){
                    o.type = 'image';
                }
                if( o.type=='detect' && dataSrc.length>3 && (dataSrc.indexOf('.mp4') == dataSrc.length-4 || dataSrc.indexOf('.m4v') == dataSrc.length-4 )){
                    o.type = 'video';
                }
                if( o.type=='detect' && dataSrc.length>3 && (dataSrc.indexOf('.mp3') == dataSrc.length-4)){
                    o.type = 'audio';
                }

                if(o.type=='detect'){

                    if(dataSrc.indexOf('.mpd')>String(dataSrc).length-5){
                ...
                if(o.type=='detect' && dataSrc.indexOf('youtube.com/embed/') > -1 ){
                    var auxa = dataSrc.split('youtube.com/embed/');
                    //console.info(auxa);
                    dataSrc = auxa[1];
                    o.type = 'youtube';
                }

                if(o.type=='detect' && dataSrc.indexOf('youtube.com/watch?v=') > -1 ){
                    dataSrc = (get_query_arg(dataSrc, 'v'));
                    o.type = 'youtube';
                }

                if(o.type=='detect' && dataSrc.indexOf('vimeo.com/') > -1 ){

//                    console.info(dataSrc, dataSrc.split('vimeo.com/'));
                    var auxa = dataSrc.split('vimeo.com/');
                    dataSrc = auxa[1];
                    o.type='vimeo';
                }
                ...
                if(o.settings_video_overlay=='on' && (is_iPad==false || (is_iPad && o.type!='youtube') ) ){
                    ...
                    if(cthis.hasClass('zoomvideoplayer-is-ad') && cthis.attr('data-ad_link')){


                        _theMediaCon.after('<a target="_blank" href="'+cthis.attr('data-ad_link')+'" class="video-overlay"></a>');
...
//                        console.info('api_reinit_skipad');
                    ...
                    cthis.on('click', '.cover-play-btn', handle_mouse);
...
//                    cthis.bind('webkitfullscreenchange mozfullscreenchange fullscreenchange', change_fullscreen);
                ...
                $(document).off('click.dzszvp');
                $(document).on('click.dzszvp','.zoomvideoplayer-is-ad .video-overlay', function(){

                    var _t = $(this);


                    if(_t.parent().attr('data-ad_link')){


                        if(_t.parent().get(0) && _t.parent().get(0).api_seek_to){

                            _t.parent().get(0).api_seek_to(0.99);

                            setTimeout(function(arg){

                                if(arg.get(0) && arg.get(0).api_pause_media){
                                    arg.get(0).api_pause_media();
                                }
                            },1500,_t.parent().parent());
                        }
                    }
            ...
            function create_ad(pargs,ind){
                ...
                console.info('create_ad()', margs,ind);

                if(margs.type=='iframe'){

                    var aux='<div id="'+cid+'_ad_'+ind+'" class=" zoomvideoplayer-ad-element '+''+'" data-source="'+margs.source+'"';


                    aux+='>';

                    aux+='<iframe src="'+margs.source+'"" style="width: 100%; height:100%;     position: absolute; top: 0; left: 0;"></iframe>';


                    aux+='</div>';
                    var auxtest = cthis.appendOnce(aux, 'zoomvideoplayer-ad-element');




                    ad_skipad_time = margs.ad_skip_delay;
...
//                console.info(cthis, 'create_ad');
                    var aux='<div id="'+cid+'_ad_'+ind+'" class="zoomvideoplayer zoomvideoplayer-is-ad '+ o.design_skin+'" data-source="'+margs.source+'"';

                    if(margs.sourceogg){
                        aux+=' data-sourceogg="'+margs.sourceogg+'"';
                    }

                    if(margs.ad_type){
                        aux+=' data-type="'+margs.ad_type+'"';
                    }
                    if(margs.ad_skip_delay){
                        aux+=' data-ad_skip_delay="'+margs.ad_skip_delay+'"';
                    }
                    if(margs.ad_link){
                        aux+=' data-ad_link="'+margs.ad_link+'"';
                    }
...
//                        console.info(cthis, _videoPlayerAd);
            ...
            function init_dash(){
                ...
                console.info('init_dash() ');
                console.info('_theMedia_ - ',_theMedia_);
                dash_context = new Webm.di.WebmContext();
                dash_player = new MediaPlayer(dash_context);
                dash_player.startup();
                dash_player.attachView(_theMedia_);
            ...
            function init_readyControls(){
                ...
                if (o.type == 'dash') {
                    ...
                    if(window.Webm){
                        init_dash();
                        //console.info(_theMedia_.naturalWidth);
                    }else{
...
//                    console.info(can_play_mp4(), !is_firefox(), o.settings_rtmpServer,o.settings_rtmpServer!='' && (is_safari() || is_ios()))
                    if(can_play_mp4() && (o.settings_rtmpServer=='' || (o.settings_rtmpServer!='' && (is_safari() || is_ios() || is_android() )))){
                        videolayer = '<video class="the-media" preload="metadata"';
                        videolayer+='>';
                ...
                if(o.type=='image'){
                    ...
                    var videolayer = '';
                    videolayer = '<div class="media-image" style="background-image: url('+dataSrc+'); "></div>';
                    _theMediaCon.append(videolayer);
                }
...
//                    console.info(o.autoplay)
                    ...
                    _theMediaCon.append('<iframe width="100%" height="100%" src="' + o.settings_httpprotocol + '//player.vimeo.com/video/' + dataSrc + '?title=0&amp;byline=0&amp;portrait=0&amp;badge=0&amp;color=ff9933'+str_autoplay+'" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen style=""></iframe>');
...
//                }
                ...
                if(o.type=='video' || o.type=='audio' || o.type=='image' || o.type=='dash'){

//                    console.info(cthis, _theMedia_.duration);
                    if(_theMedia_ && typeof (_theMedia_.duration)!='undefined'){
...
//                    console.info(isAndroid, is_iPad);
                ...
                inter_check_every_1000ms = setInterval(check_every_1000ms,1000);
...
//                console.info(o.autoplay);
                ...
                if (o.type == 'video'||o.type == 'audio'|| o.type == 'dash') {
                    if(!isflash){
...
//                console.info('reinit_skipad');
                ...
                if(ad_skipad_time!=1001){
                    _adSkipCon.removeClass('skipable');
                    setTimeout(function(){
//                            console.info(cthis.find('.skipad-con').eq(0))
                        time_counter_skipad=0;
                        _adSkipCon.addClass('skipable');
                        ...
                        _adSkipCon.children('.skipad').bind('click', function() {
                            ...
                            if(cthis.hasClass('zoomvideoplayer-is-ad')) {
                                ...
                                ad_playing = false;
                                cthis.find('.zoomvideoplayer-ad-element,.skipad-con').remove();
                                if(wasPlaying){
                                    play_media();
                                }
...
//--normal video on modern browsers
                if (o.settings_enableTags == 'on') {
                    cthis.find('.dzstag-tobe').each(function() {
                        ...
                        var aux_l = _t.attr('data-left');
                        var aux_t = _t.attr('data-top');
                        ...
                        // console.info(_t, aux_l, aux_t);
                        _t.css({'left': (aux_l + ''), 'top': (aux_t + '')});
                        //console.log(_t);
                ...
                if (o.settings_enableTags == 'on') {
                    var myVar = setInterval(check_tags, 1000);
                }
                        ...
                        arrTags.each(function() {
                            ...
                            if(_t.attr('data-pauseonactive')=='on'){
                                ...
                                if (Number(_t.attr('data-starttime')) <= roundTime && Number(_t.attr('data-endtime')) >= roundTime) {
                                    $('body').addClass('restriction-body');
                                    $('#restriction-view')[0].click();
                                    $('#edit-price-close').addClass('video-rest');
                                    pause_media();
                                    clearInterval(myVar);
                                   // _t.addClass('active');
                                }else{
...
//                    console.info(_t.css("left"));
            ...
            function check_every_1000ms(){

                // console.info('check_every_1000ms - ','ad_array - ',ad_array);
...
//                        console.info(_theMedia_);
                ...
                if (o.type == 'youtube' && typeof _theMedia_.getCurrentTime !='undefined') {
                    ...
                    if(_scrubBg){
                        ...
                        if(_theMedia_.getVideoLoadedFraction){
...
//                    aux4+='<span class="social-container--bg"></span>';
                    ...
                    console.info('_t - ', _t, _t.hasClass('cover-play-btn'));
                    if(_t.hasClass('play-btn') || _t.hasClass('cover-play-btn')){
                       console.info(e, _t);
                        play_media();
                    ...
                    if(_t.hasClass('video-overlay') || _t.hasClass('cover-play-btn')){
                        if(ispaused){
                            play_media();
                        }else{
                            pause_media();
...
//                        console.info(_t);
            ...
            function play_media(){
               ...
               console.info('play_media()' , cthis, ad_playing, hasAutoplayed, o.settings_suggestedQuality, currQuality);
...
function get_base_url_arr(){
    ...
    for(var i23 in scripts){
        if(scripts[i23] && scripts[i23].src && String(scripts[i23].src).indexOf('zoomplayer.js')>-1){
