...
$sql1 = "Select * from package_list where pkg_id  = '".$request->packageid."'";

$result1 = mysqli_query($con,$sql1);

$package = mysqli_fetch_assoc($result1);

if($package) {

    $itemPrice = '';
    if($package['sub_type'] == 'None'){
        $itemPrice = '0';
    } elseif($package['sub_type'] == 'Day'){
        $itemPrice = $package['sub_price_day'];
    } elseif($package['sub_type'] == 'Week'){
        $itemPrice = $package['sub_price_week'];
    } elseif($package['sub_type'] == 'Month'){
        $itemPrice = $package['sub_price_month'];
    } else{
        $itemPrice = $package['sub_price_year'];
    } 

    $packageid = $request->packageid;

    $itemName = $package['pkg_name'];
    ...
    if($request->isAddress){
	    try {
	        ...
	        $session = \Stripe\Checkout\Session::create([ 
	            ...
	            $customer_address,
	            'mode' => 'payment', 
	            'success_url' => 'https://shareio.com/chk-package-payment-response.php?session_id={CHECKOUT_SESSION_ID}&&packageid='.$packageid.'&&itemPrice='.$itemPrice.'',
	            'cancel_url' => 'https://shareio.com/chk-package-payment-response.php', 
	        ]);
