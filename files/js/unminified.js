...
(function (window){
	...
	var FWDRAP = function(props){
		...
		_s.setStageContainerHeight = function(animate){
			...
			if(_s.openInPopup_bl){
				...
				if(_s.playlist_do){
					...
					if(animate){
						//FWDAnimation.to(_s.playlist_do, .8, {y:_s.controller_do.h, delay:.4, ease:Expo.easeInOut});
					}else{
		...
		_s.playPauseClickHandler = function(e){
			...
			//if(_s.controller_do.mainHolder_do.y != 0 && _s.isMbl) return;
		...
		_s.setupDisableClick = function(){
			...
			//_s.disableClick_do.setBkColor("#00FF00")
		};
		...
		_s.goFullScreen = function(){
			...
			//}else{
				//_s.videosHolder_do.getStyle().position = "fixed";
				//_s.videosHolder_do.getStyle().zIndex = 9999999999998;
			//}
		...
		_s.addmain_doToTheOriginalParent = function(){
			...
			//_s.dispatchEvent(FWDRAP.GO_NORMALSCREEN);
		};
		...
		_s.onPreloaderLoadDone = function(){
			...
			if((_s._d.useContinuousPlayback_bl || _s._d.autoPlay_bl) && !FWDRAP.iFrame && FWDRAPUtils.isChrome && !FWDRAPUtils.isMobile){
				FWDRAP.iFrame = document.createElement("iframe");
				FWDRAP.iFrame.src = _s._d.mainFolderPath_str + 'audio/silent.mp3';
				FWDRAP.iFrame.style.position = 'absolute';
				FWDRAP.iFrame.style.top = '-500px';
				document.documentElement.appendChild(FWDRAP.iFrame);
			}
		...
		_s.showPlaybacrateWindowHandler = function(e){
			//_s.playbackRateWindow_do.positionAndResize();
	   ...
	   _s.removeListener = function(type, listener){
		...
		if(_s.useYoutube_bl){	
			if((location.protocol.indexOf("file:") != -1 && FWDRAPUtils.isIE)
			   ...
			   //|| (location.protocol.indexOf("file:") != -1 && FWDRAPUtils.isSafari)
...
(function(window){
	...
	var FWDRAPAudioData = function(props, playListElement, prt){
		...
		//_s.sc_ar = props.soundCloudAPIKey || "0,1, 2, 3,4";
		...
		_s.shuffle_bl = false;
		_s.showLoopButton_bl = false;
		_s.showShuffleButton_bl = false;
		_s.showDownloadMp3Button_bl = false;
		_s.showBuyButton_bl = false;
		_s.showPlaylistsButtonAndPlaylists_bl = false;
		_s.showPlaylistsByDefault_bl = false;
		...
		_s.parseProperties = function(){
			...
			//if(_s.useVectorIcons) _s.useHEX = false;
			...
			//if(_s.isMbl) _s.allowToChangeVolume_bl = false;
			...
			//if(FWDRAPUtils.isMobile) _s.autoPlay_bl = false;
		...
		_s.JSONPRequestTimeoutError = function(){
			...
			_s.isPlaylistDispatchingError_bl = true;
			showLoadPlaylistErrorId_to = setTimeout(function(){
				_s.dispatchEvent(FWDRAPData.LOAD_ERROR, {text:"Error loading youtube playlist!<font color='#ff0000'>" + _s.youtubeUrl_str + "</font>"});
				_s.isPlaylistDispatchingError_bl = false;
			}, 50);
			return;
		};
		...
		_s.parseIcecastRadioURL = function(object){
			...
			//_s.dispatchEvent(FWDRAPAudioData.UPDATE_SERVER_INFO, {text:infoWindowStr});
		...
		_s.ajaxOnLoadHandler = function(e){
			...
			if(_s.xhr.readyState == 4){
				...
				if(_s.xhr.status == 401){
					...
					//console.log(_s.xhr.responseText)
		...
		_s.parseDOMPlaylist = function(idOrObject){
			...
			for(var i=0; i<totalChildren; i++){
				...
				if(i > _s.maxPlaylistItems - 1) break;
				//console.log(FWDRAPUtils.getAttributeValue(child, "data-type"));
...
(function(window){
	...
	var FWDRAPAudioScreen = function(volume){
		...
		_s.setSource = function(sourcePath){
			...
			var formats_ar = FWDRAP.getAudioFormats;
			...
			for(var i=0; i<paths_ar.length; i++){
				var path = paths_ar[i];
				paths_ar[i] = FWDRAPUtils.trim(path);
			}
...
(function (window){
	...
	var FWDRAPComboBox = function(prt, props_obj){
		...
		_s.selector_do = null;
		_s.mainButtonsHolder_do = null;
		_s.buttonsHolder_do = null;
		...
		_s.buttonOnMouseDownHandler = function(e){
			...
			_s.curId = e.id;
			//_s.setButtonsStateBasedOnId(_s.curId);
		...
		_s.hideComplete = function(){
			...
			//if(_s.isButtonCliecked_bl) _s.dispatchEvent(FWDRAPComboBox.BUTTON_PRESSED, {id:_s.curId});
			//_s.isButtonCliecked_bl = false;
		}
		...
		_s.scrollBarTouchStartHandler = function(e){
			...
			if(_s.hasPointerEvent_bl){
				...
				window.addEventListener("touchmove", _s.scrollBarTouchMoveHandler);
			}
		...
		_s.setupScrollbar = function(){
			...
			/*if(_s.useHEX){
				_s.scrHandlerLinesN_do = new FWDRAPDisplayObject("div");
				_s.scrHandlerLinesN_do.setWidth(_s.playlistScrLines_img.width);
				_s.scrHandlerLinesN_do.setHeight(_s.playlistScrLines_img.height);
				_s.mainhandlerN_canvas = FWDRAPUtils.getCanvasWithModifiedColor(_s.playlistScrLines_img, _s.sBC).canvas;
			...
			/*if(_s.useHEX){
				_s.scrHandlerLinesS_do = new FWDRAPDisplayObject("div");
				_s.scrHandlerLinesS_img.onload = function(){
					...
					_s.scrubberLines_s_canvas = FWDRAPUtils.getCanvasWithModifiedColor(_s.scrHandlerLinesS_img, _s.sBC, true);
				...
				//_s.scrHandlerMiddle_do.getStyle().background = "url('" + _d.scrLinesSPath_str + "')";
		...
		_s.destroy = function(){
			...
			//for(var i=0; i<_s.totalButtons; i++) _s.buttons_ar[i].destroy();
			
			_s.mainHolder_do.destroy();
			_s.selector_do.destroy();
			_s.mainButtonsHolder_do.destroy();
			_s.buttonsHolder_do.destroy();
			
			_s.categories_ar = null;
			_s.buttons_ar = null;
			_s.mainHolder_do = null;
			_s.selector_do = null;
			_s.mainButtonsHolder_do = null;
			_s.buttonsHolder_do = null;
			_s.upArrowN_img = null;
			_s.upArrowS_img = null;
...
var FWDRAPComboBoxSelector = function(
		...
		_s.setupMainContainers = function(){
			...
			if(_s.useHEX){
				...
				_s.arrowN_img.onload = function(){
					//_s.nBC =  _s.backgroundNormalColor_str;
		...
		_s.setNormalState = function(animate, removeDelay){
			//if(FWDAnimation.isTweening(_s.arrowS_sdo)) return;
			var dll = .6;
		...
		_s.destroy = function(){
			...
			FWDAnimation.killTweensOf(_s.text_sdo);
			FWDAnimation.killTweensOf(_s.colorObj);
			_s.text_sdo.destroy();
...
var FWDRAPController = function(
		...
		_s.loopButton_do = null;
		_s.shuffleButton_do = null;
		_s.downloadButton_do = null;
		_s.shareButton_do = null;
		_s.simpleText_do = null;
		_s.animText1_do = null;
		...
		_s.loopButtonToolTip_do = null;
		_s.shuffleButtonToolTip_do = null;
		_s.shareButtonToolTip_do = null;
		_s.downloadButtonToolTip_do = null;
		_s.buyButtonToolTip_do = null;
		...
		//##########################################//
		_s.init = function(){
			//if(prt.useVideo_bl || prt.useYoutube_bl){
		...
		_s.goNormalScreen = function(){
			...
			//_s.mainHolder_do.addChildat(_s.mainBk_do, 0);
		...
		_s.positionButtons = function(){
			...
			if(prt.isFullScreen_bl){
				...
				for(var i=0; i<totalButtons; i++){
					...
					if(i == 0){
						...
						//console.log(prevItem.w)
						if(button == _s.totalTime_do){
							//console.log(prevItem == _s.mainScrubber_do)
							//console.log(prevItem.w)
						}
						button.setY(parseInt((_s.controllerHeight - button.h)/2));
					}
			...
			if(leftWidth > _s.minLeftWidth){
				...
				//if(isNaN(totalButtonsWidthWithCustoRAPace)) return
		...
		_s.setupMainScrubber = function(){
			...
			//}else{
				//_s.mainScrubberBkMiddle_do = new FWDRAPDisplayObject("img");
		...
		_s.setupTime = function(){
			...
			//_s.totalTime_do.setBkColor("#FF0000");
		...
		_s.setupVolumeScrubber = function(){
			...
			//}else{
				//_s.volumeScrubberBkMiddle_do = new FWDRAPDisplayObject("img");
			...
			if(_s.allowToChangeVolume_bl){
				if(_s.isMbl){
					...
					_s.volumeScrubber_do.screen.attachEvent("onmouseover", _s.volumeScrubberOnOverHandler);
					_s.volumeScrubber_do.screen.attachEvent("onmouseout", _s.volumeScrubberOnOutHandler);
					_s.volumeScrubber_do.screen.attachEvent("onmousedown", _s.volumeScrubberOnDownHandler);
				}
			}
...
(function (window){
	...
	var FWDRAPPlaybackRateWindow = function(_d, prt){
		...
		_s.setupScrubber = function(){
			...
			//}else{
				//_s.scrubberBkMiddle_do = new FWDRAPDisplayObject("img");
...
(function(){
	var FWDRAPPlaylist = function(
		...
		_s.setupInput = function(){
			...
			/*if(_s.useHEX){
				_s.inputArrow_do = new FWDRAPDisplayObject("div");
				_s.inputArrow_img.onload = function(){		
					_s.mainScrubberDragLeft_canvas = FWDRAPUtils.getCanvasWithModifiedColor(_s.inputArrow_img, _s.nBC).canvas;
					_s.inputArrow_do.setWidth(_s.inputArrow_img.width);
					_s.inputArrow_do.setHeight(_s.inputArrow_img.height);
					_s.inputArrow_do.screen.appendChild(_s.mainScrubberDragLeft_canvas);
				}					
			}else{*/
				_s.inputArrow_do = new FWDRAPDisplayObject("img"); 
		...
		_s.positionSearchBar = function(){
			...
			inputWidth = _s.sW - (_s.startSpaceBetweenButtons * 2) - _s.inputArrow_do.w - 12;
		...
		_s.updateScrollBarHandlerAndContent = function(animate, overwrite){
			...
			//console.log("_s.prevSortId " + _s.prevSortId + " ---- " + "_s.sortId" + _s.sortId);
		...
		_s.mouseWheelHandler = function(e){
			...
			leftId = parseInt(_s.playListFinalY/_s.itemHeight);
		...
		_s.setupMobileScrollbar = function(){
			if(_s.hasPointerEvent_bl){
				...
				_s.itemsHolder_do.screen.addEventListener("touchstart", _s.scrollBarTouchStartHandler);
			}
			_s.updateMobileScrollBarId_int = setInterval(_s.updateMobileScrollBar, 16);
		};
...
(function(window){
	...
	var FWDRAPYoutubeScreen = function(prt, volume){
		...
		_s.init = function(){
			...
			//prt.main_do.addChild(_s);
			//console.log(prt.controller_do.thumb_do)
			prt.main_do.addChild(_s);
			_s.resizeAndPosition();
			_s.setupVideo();
		...
		_s.stateChangeHandler = function(e){
			//logger.log(e.data + " " + prt.instanceName_str + " " + _s.isCued_bl)
			//console.log(e.data + " " + prt.instanceName_str + " " + _s.isCued_bl)
		...
		_s.stop = function(){
			if(_s.isStopped_bl) return;
			//logger.log("# YTB stop #" + prt.instanceName_str);
			_s.isPlaying_bl = false;
			_s.isStopped_bl = true;
			_s.isCued_bl = false;
			_s.allowScrubing_bl = false;
