...
function sentOrderMail($email,$subject,$message){
...
function sentOrderMailAdmin($adminemail,$CollectiveSubject,$CollectiveMessage){
	
$html               = $CollectiveMessage;
...
$mail->Password     = SENDGRID_PASSWORD;
$mail->SetFrom($mail_from, $mail_from_name);
$mail->Subject      = $CollectiveSubject;
$mail->Body         = $html;

$conn = new mysqli('mysql.islandpharmz.shop', 'islandpharmzshop', 'D3icqXhm','staging');
$sel_loc = "SELECT * FROM email_setting WHERE id = '1'";	
$result = $conn->query($sel_loc);
if($result->num_rows > 0){ 
while($row = $result->fetch_assoc()){
		$email_1 = $row['ccemail'];
		$variableAry=explode(",",$email_1); 
