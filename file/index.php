...
$_SESSION['contenturl'] = 'file/'.$_REQUEST['file'];
...
<body class="<?php if($priceResultinfo['file_type'] == "video/mp4" || $priceResultinfo['file_type'] == "video/webm") { echo "video-view"; }?>">
  ...
  if(mysqli_num_rows($result) > 0){
    ...
    if(mysqli_num_rows($user_access_check_result) > 0){
      ...
      if(isset($_COOKIE['purchased'.$priceResultinfo['file_id']])){
        ...
        setcookie('purchased'.$priceResultinfo['file_id'], '', time()-3600, "/file/"); 
      }
    }
        ...
        $.ajax({
            ...
            success: function(resp){ 
              ...
              var unlink = setInterval(function(){ 
                  $.ajax({
                  url: 'unlink.php',
                  data: {'file' : "<?php echo '/www/wwwroot/contentshare.me/pdf/'.$temp_pdffile_name;?>" },
      ...
      <script>
       window.onload = function viewimage() {
        ...
        $.ajax({
            ...
            success: function(resp){
              //resp.contents().find('.noSel smooth_zoom_preloader').css('height', '100%'); 
              $('#content').html(resp);
              if(useraccess == 'No'){ 
                if(file_download == '1'){
                if(content_price < 1){
