(function(f){if(typeof exports==="object"&&typeof module!=="undefined"){module.exports=f()}else if(typeof define==="function"&&define.amd){define([],f)}else{var g;if(typeof window!=="undefined"){g=window}else if(typeof global!=="undefined"){g=global}else if(typeof _s!=="undefined"){g=_s}else{g=this}g.jsmediatags = f()}})(function(){var define,module,exports;return (function e(t,n,r){function s(o,u){if(!n[o]){if(!t[o]){var a=typeof require=="function"&&require;if(!u&&a)return a(o,!0);if(i)return i(o,!0);var f=new Error("Cannot find module '"+o+"'");throw f.code="MODULE_NOT_FOUND",f}var l=n[o]={exports:{}};t[o][0].call(l.exports,function(e){var n=t[o][1][e];return s(n?n:e)},l,l.exports,e,t,n,r)}return n[o].exports}var i=typeof require=="function"&&require;for(var o=0;o<r.length;o++)s(r[o]);return s})({1:[function(require,module,exports){
...
function _possibleConstructorReturn(_s, call) { if (call && (_typeof(call) === "object" || typeof call === "function")) { return call; } return _assertThisInitialized(_s); }

function _getPrototypeOf(o) { _getPrototypeOf = Object.setPrototypeOf ? Object.getPrototypeOf : function _getPrototypeOf(o) { return o.__proto__ || Object.getPrototypeOf(o); }; return _getPrototypeOf(o); }

function _assertThisInitialized(_s) { if (_s === void 0) { throw new ReferenceError("this hasn't been initialised - super() hasn't been called"); } return _s; }
...
function (_MediaFileReader) {
  ...
  _createClass(BlobFileReader, [{
    ...
    key: "loadRange",
    value: function loadRange(range, callbacks) {
      var _s = this; // $FlowIssue - flow isn't aware of mozSlice or webkitSlice
      ...
      browserFileReader.onloadend = function (event) {
        ...
        _s._fileData.addData(range[0], intArray);
...
/**
 * This class represents a file that might not have all its _d loaded yet.
 ...
 * exception is when the _d is not available yet, an error will be thrown.
 * This class does not load the _d, it just manages it. It provides operations
 * to add and read _d from the file.
 *
 * 
 */
'use strict';
...
function () {
  ...
  /**
   * Adds _d to the file storage at a specific offset.
   */


  _createClass(ChunkedFileData, [{
    key: "addData",
    value: function addData(offset, _d) {
      var offsetEnd = offset + _d.length - 1;
      ...
      if (chunkRange.startIx === NOT_FOUND) {
        ...
        // If the _d to add collides with existing chunks we prepend and
        // append _d from the half colliding chunks to make the collision at
        // 100%. The new _d can then replace all the colliding chunkes.
        ...
        var needsAppend = offsetEnd < lastChunk.offset + lastChunk._d.length - 1;
        var chunk = {
        ...
        if (needsPrepend) {
          var slicedData = this._sliceData(firstChunk._d, 0, offset - firstChunk.offset);

          chunk._d = this._concatData(slicedData, _d);
        }

        if (needsAppend) {
          // Use the lastChunk because the slice logic is easier to handle.
          var slicedData = this._sliceData(chunk._d, 0, lastChunk.offset - chunk.offset);

          chunk._d = this._concatData(slicedData, lastChunk._d);
        }
    ...
    key: "_sliceData",
    value: function _sliceData(_d, begin, end) {
      // Some TypeArray implementations do not support slice yet.
      if (_d.slice) {
        return _d.slice(begin, end);
      } else {
        // $FlowIssue - flow thinks _d is a string but it's not
        return _d.subarray(begin, end);
      }
    }
    /**
     ...
     * When no chunks are found `insertIx` denotes the index where the _d
     * should be inserted in the _d list (startIx == NOT_FOUND and endIX ==
     * NOT_FOUND).
     */
    ...
    value: function _getChunkRange(offsetStart, offsetEnd) {
      ...
      for (var i = 0; i < this._fileData.length; i++, insertIx = i) {
        ...
        var chunkOffsetEnd = chunkOffsetStart + this._fileData[i]._d.length;
        ...
        // we're going end up with a single block with all contiguous _d.
    ...
    value: function hasDataRange(offsetStart, offsetEnd) {
      for (var i = 0; i < this._fileData.length; i++) {
        ...
        if (offsetStart >= chunk.offset && offsetEnd < chunk.offset + chunk._d.length) {
          return true;
        }
      }
    ...
    value: function getByteAt(offset) {
      ...
      for (var i = 0; i < this._fileData.length; i++) {
        ...
        var dataChunkEnd = dataChunkStart + this._fileData[i]._d.length - 1;
      ...
      if (dataChunk) {
        return dataChunk._d[offset - dataChunk.offset];
      }
...
/* FLAC metadata is stored in blocks containing _d ranging from STREAMINFO to
 ...
 * it is the last metadata block before the audio _d and what the block type is.
...
function (_MediaTagReader) {
  ...
  _createClass(FLACTagReader, [{
    ...
    /**
     * Function called to load the _d from the file.
     *
    ...
    value: function _loadData(mediaFileReader, callbacks) {
      ...
      mediaFileReader.loadRange([4, 7], {
        onSuccess: function onSuccess() {
          _s._loadBlock(mediaFileReader, 4, callbacks);
        }
      });
    }
    /**
    ...
    value: function _loadBlock(mediaFileReader, offset, callbacks) {
      ...
      if (COMMENT_HEADERS.indexOf(blockHeader) !== -1) {
        ...
        mediaFileReader.loadRange([offsetMetadata, offsetMetadata + blockSize], {
          onSuccess: function onSuccess() {
            _s._commentOffset = offsetMetadata;

            _s._nextBlock(mediaFileReader, offset, blockHeader, blockSize, callbacks);
          }
        });
        ...
        mediaFileReader.loadRange([offsetMetadata, offsetMetadata + blockSize], {
          onSuccess: function onSuccess() {
            _s._pictureOffset = offsetMetadata;
     ...
     * will call the success callback, allowing _d parsing to begin.
     * @param {MediaFileReader} mediaFileReader - The MediaFileReader used to parse the file.
    ...
    value: function _nextBlock(mediaFileReader, offset, blockHeader, blockSize, callbacks) {
      ...
      if (blockHeader > 127) {
        ...
        mediaFileReader.loadRange([offset + 4 + blockSize, offset + 4 + 4 + blockSize], {
          onSuccess: function onSuccess() {
            _s._loadBlock(mediaFileReader, offset + 4 + blockSize, callbacks);
          }
        });
      }
    }
    /**
     * Parses the _d and returns the tags.
     *
     ...
     * @param {MediaFileReader} _d - The MediaFileReader to parse the file with.
    ...
    key: "_parseData",
    value: function _parseData(_d, tags) {
      var vendorLength = _d.getLongAt(this._commentOffset, false);
       ...
       */
      // var vendor = _d.getStringWithCharsetAt(offsetVendor, vendorLength, "utf-8").toString();
       ...
       *
       * As we loop through all of the comment blocks, we get the _d length in order to
      ...
      var numComments = _d.getLongAt(offsetList, false);
      var dataOffset = offsetList + 4;
      var title, artist, album, track, genre, picture;

      for (var i = 0; i < numComments; i++) {
        var _dataLength = _d.getLongAt(dataOffset, false);

        var s = _d.getStringWithCharsetAt(dataOffset + 4, _dataLength, "utf-8").toString();
        var d = s.indexOf("=");
       ...
       *
       * All the lengths for the picture _d can be found online here:
       * https://xiph.org/flac/format.html#metadata_block_picture
       */


      if (this._pictureOffset) {
        var imageType = _d.getLongAt(this._pictureOffset, true);
        var offsetMimeLength = this._pictureOffset + 4;
        var mimeLength = _d.getLongAt(offsetMimeLength, true);
        var offsetMime = offsetMimeLength + 4;
        var mime = _d.getStringAt(offsetMime, mimeLength);
        var offsetDescriptionLength = offsetMime + mimeLength;
        var descriptionLength = _d.getLongAt(offsetDescriptionLength, true);
        var offsetDescription = offsetDescriptionLength + 4;
        var description = _d.getStringWithCharsetAt(offsetDescription, descriptionLength, "utf-8").toString();
        var offsetDataLength = offsetDescription + descriptionLength + 16;
        var dataLength = _d.getLongAt(offsetDataLength, true);
        var offsetData = offsetDataLength + 4;
        var imageData = _d.getBytesAt(offsetData, dataLength, true);
        picture = {
          format: mime,
...
function (_MediaTagReader) {
  ...
  _createClass(ID3v1TagReader, [{
    ...
    key: "_parseData",
    value: function _parseData(_d, tags) {
      var offset = _d.getSize() - 128;
      var title = _d.getStringWithCharsetAt(offset + 3, 30).toString();
      var artist = _d.getStringWithCharsetAt(offset + 33, 30).toString();
      var album = _d.getStringWithCharsetAt(offset + 63, 30).toString();
      var year = _d.getStringWithCharsetAt(offset + 93, 4).toString();
      var trackFlag = _d.getByteAt(offset + 97 + 28);
      var track = _d.getByteAt(offset + 97 + 29);

      if (trackFlag == 0 && track != 0) {
        var version = "1.1";
        var comment = _d.getStringWithCharsetAt(offset + 97, 28).toString();
      } else {
        var version = "1.0";
        var comment = _d.getStringWithCharsetAt(offset + 97, 30).toString();
        track = 0;
      }

      var genreIdx = _d.getByteAt(offset + 97 + 30);
    ...
    value: function getTagIdentifierByteRange() {
      ...
      // fetch for the tag identifier and another for the _d, we load the
      // entire _d since it's so small.
      return {
        offset: -128,
        length: 128
      };
...
function () {
  ...
  _createClass(ID3v2FrameReader, null, [{
    ...
    key: "readFrames",
    value: function readFrames(offset, end, _d, id3header, tags) {
      var frames = {};
      ...
      offset < end - frameHeaderSize) {
        var header = this._readFrameHeader(_d, offset, id3header);
        ...
        var frameData = _d; // console.log(offset, frameId, header.size + header.headerSize, flags && flags.format.unsynchronisation);
        // advance _d offset to the next frame _d
        ...
        } // the first 4 bytes are the real _d size
    ...
    value: function _readFrameHeader(_d, offset, id3header) {
      var major = id3header.major;
      var flags = null;
      ...
      switch (major) {
        case 2:
          var frameId = _d.getStringAt(offset, 3);
          var frameSize = _d.getInteger24At(offset + 3, true);
          break;

        case 3:
          var frameId = _d.getStringAt(offset, 4);
          var frameSize = _d.getLongAt(offset + 4, true);
          break;

        case 4:
          var frameId = _d.getStringAt(offset, 4);
          var frameSize = _d.getSynchsafeInteger32At(offset + 4);
          break;
      }
      ...
      if (frameId) {
        ...
        if (major > 2) {
          flags = this._readFrameFlags(_d, offset + 8);
        }
      }
    ...
    key: "_readFrameFlags",
    value: function _readFrameFlags(_d, offset) {
      return {
        message: {
          tag_alter_preservation: _d.isBitSetAt(offset, 6),
          file_alter_preservation: _d.isBitSetAt(offset, 5),
          read_only: _d.isBitSetAt(offset, 4)
        },
        format: {
          grouping_identity: _d.isBitSetAt(offset + 1, 7),
          compression: _d.isBitSetAt(offset + 1, 3),
          encryption: _d.isBitSetAt(offset + 1, 2),
          unsynchronisation: _d.isBitSetAt(offset + 1, 1),
          data_length_indicator: _d.isBitSetAt(offset + 1, 0)
        }
      };
    }
  }, {
    ...
    value: function getUnsyncFileReader(_d, offset, size) {
      var frameData = _d.getBytesAt(offset, size);
...
frameReaderFunctions['APIC'] = function readPictureFrame(offset, length, _d, flags, id3header) {
  var start = offset;
  var charset = getTextEncoding(_d.getByteAt(offset));

  switch (id3header && id3header.major) {
    case 2:
      var format = _d.getStringAt(offset + 1, 3);
      offset += 4;
      break;

    case 3:
    case 4:
      var format = _d.getStringWithCharsetAt(offset + 1, length - 1);
  ...
  var bite = _d.getByteAt(offset);
  var type = PICTURE_TYPE[bite];
  var desc = _d.getStringWithCharsetAt(offset + 1, length - (offset - start) - 1, charset);
  offset += 1 + desc.bytesReadCount;
  return {
    "format": format.toString(),
    "type": type,
    "description": desc.toString(),
    "_d": _d.getBytesAt(offset, start + length - offset)
  };
}; // ID3v2 chapters according to http://id3.org/id3v2-chapters-1.0


frameReaderFunctions['CHAP'] = function readChapterFrame(offset, length, _d, flags, id3header) {
  var originalOffset = offset;
  var result = {};
  var id = StringUtils.readNullTerminatedString(_d.getBytesAt(offset, length));
  result.id = id.toString();
  offset += id.bytesReadCount;
  result.startTime = _d.getLongAt(offset, true);
  offset += 4;
  result.endTime = _d.getLongAt(offset, true);
  offset += 4;
  result.startOffset = _d.getLongAt(offset, true);
  offset += 4;
  result.endOffset = _d.getLongAt(offset, true);
  offset += 4;
  var remainingLength = length - (offset - originalOffset);
  result.subFrames = this.readFrames(offset, offset + remainingLength, _d, id3header);
  return result;
}; // ID3v2 table of contents according to http://id3.org/id3v2-chapters-1.0


frameReaderFunctions['CTOC'] = function readTableOfContentsFrame(offset, length, _d, flags, id3header) {
  var originalOffset = offset;
  var result = {
    childElementIds: [],
    id: undefined,
  ...
  result.id = id.toString();
  offset += id.bytesReadCount;
  result.topLevel = _d.isBitSetAt(offset, 1);
  result.ordered = _d.isBitSetAt(offset, 0);
  offset++;
  result.entryCount = _d.getByteAt(offset);
  offset++;

  for (var i = 0; i < result.entryCount; i++) {
    var childId = StringUtils.readNullTerminatedString(_d.getBytesAt(offset, length - (offset - originalOffset)));
...
frameReaderFunctions['COMM'] = function readCommentsFrame(offset, length, _d, flags, id3header) {
  var start = offset;
  var charset = getTextEncoding(_d.getByteAt(offset));
  var language = _d.getStringAt(offset + 1, 3);
  var shortdesc = _d.getStringWithCharsetAt(offset + 4, length - 4, charset);
  offset += 4 + shortdesc.bytesReadCount;
  var text = _d.getStringWithCharsetAt(offset, start + length - offset, charset);
  return {
    language: language,
...
frameReaderFunctions['PIC'] = function (offset, length, _d, flags, id3header) {
  return frameReaderFunctions['APIC'](offset, length, _d, flags, id3header);
};

frameReaderFunctions['PCNT'] = function readCounterFrame(offset, length, _d, flags, id3header) {
  // FIXME: implement the rest of the spec
  return _d.getLongAt(offset, false);
};

frameReaderFunctions['CNT'] = frameReaderFunctions['PCNT'];

frameReaderFunctions['T*'] = function readTextFrame(offset, length, _d, flags, id3header) {
  var charset = getTextEncoding(_d.getByteAt(offset));
  return _d.getStringWithCharsetAt(offset + 1, length - 1, charset).toString();
};

frameReaderFunctions['TXXX'] = function readTextFrame(offset, length, _d, flags, id3header) {
  var charset = getTextEncoding(_d.getByteAt(offset));
  return getUserDefinedFields(offset, length, _d, charset);
};

frameReaderFunctions['WXXX'] = function readUrlFrame(offset, length, _d, flags, id3header) {
  if (length === 0) {
    return null;
  }
...
frameReaderFunctions['W*'] = function readUrlFrame(offset, length, _d, flags, id3header) {
  if (length === 0) {
    return null;
  }

  return _d.getStringWithCharsetAt(offset, length, 'iso-8859-1').toString();
};

frameReaderFunctions['TCON'] = function readGenreFrame(offset, length, _d, flags) {
...
frameReaderFunctions['USLT'] = function readLyricsFrame(offset, length, _d, flags, id3header) {
  var start = offset;
  var charset = getTextEncoding(_d.getByteAt(offset));
  var language = _d.getStringAt(offset + 1, 3);
  var descriptor = _d.getStringWithCharsetAt(offset + 4, length - 4, charset);
  offset += 4 + descriptor.bytesReadCount;
  var lyrics = _d.getStringWithCharsetAt(offset, start + length - offset, charset);
  return {
    language: language,
...
frameReaderFunctions['UFID'] = function readLyricsFrame(offset, length, _d, flags, id3header) {
  var ownerIdentifier = StringUtils.readNullTerminatedString(_d.getBytesAt(offset, length));
  offset += ownerIdentifier.bytesReadCount;
  var identifier = _d.getBytesAt(offset, length - ownerIdentifier.bytesReadCount);
  return {
...
function getTextEncoding(bite) {
  ...
  return charset;
} // Handles reading description/_d from either http://id3.org/id3v2.3.0#User_defined_text_information_frame
// and http://id3.org/id3v2.3.0#User_defined_URL_link_frame


function getUserDefinedFields(offset, length, _d, charset) {
  var userDesc = _d.getStringWithCharsetAt(offset + 1, length - 1, charset);
  var userDefinedData = _d.getStringWithCharsetAt(offset + 1 + userDesc.bytesReadCount, length - 1 - userDesc.bytesReadCount, charset);
  return {
    user_description: userDesc.toString(),
    _d: userDefinedData.toString()
  };
}
...
function (_MediaTagReader) {
  ...
  _createClass(ID3v2TagReader, [{
    ...
    value: function _parseData(_d, tags) {
      var offset = 0;
      var major = _d.getByteAt(offset + 3);
      ...
      var revision = _d.getByteAt(offset + 4);
      var unsynch = _d.isBitSetAt(offset + 5, 7);
      var xheader = _d.isBitSetAt(offset + 5, 6);
      var xindicator = _d.isBitSetAt(offset + 5, 5);
      var size = _d.getSynchsafeInteger32At(offset + 6);
      offset += 10;

      if (xheader) {
        // We skip the extended header and don't offer support for it right now.
        if (major === 4) {
          var xheadersize = _d.getSynchsafeInteger32At(offset);
          offset += xheadersize;
        } else {
          var xheadersize = _d.getLongAt(offset, true); // The 'Extended header size', currently 6 or 10 bytes, excludes itself.
      ...
      if (id3.flags.unsynchronisation) {
        _d = ID3v2FrameReader.getUnsyncFileReader(_d, offset, size);
        offset = 0;
        offsetEnd = _d.getSize();
      }

      var frames = ID3v2FrameReader.readFrames(offset, offsetEnd, _d, id3, expandedTags); // create shortcuts for most common _d.
...
function (_MediaTagReader) {
  ...
  _createClass(MP4TagReader, [{
    ...
    value: function _loadData(mediaFileReader, callbacks) {
      ...
      // situation do not possess any _d and are called "container" as they only
      // contain other atoms.
      // Other atoms represent a particular set of _d, like audio, video or
      ...
      var _s = this; // Load the header of the first atom

      mediaFileReader.loadRange([0, 16], {
        onSuccess: function onSuccess() {
          _s._loadAtom(mediaFileReader, 0, "", callbacks);
        },
        onError: callbacks.onError
      });
    }
    ...
    value: function _loadAtom(mediaFileReader, offset, parentAtomFullName, callbacks) {
      ...
      var _s = this; // 8 is the size of the atomSize and atomName fields.
      ...
      // Container atoms (no actual _d)

      if (this._isContainerAtom(atomName)) {
        if (atomName == "meta") {
          // The "meta" atom breaks convention and is a container with _d.
        ...
        if (atomFullName === "moov.udta.meta.ilst") {
          ...
          mediaFileReader.loadRange([offset + 8, offset + 8 + 8], {
            onSuccess: function onSuccess() {
              _s._loadAtom(mediaFileReader, offset + 8, atomFullName, callbacks);
            },
            onError: callbacks.onError
          });
        }
        ...
        mediaFileReader.loadRange([offset + atomSize, offset + atomSize + 8], {
          onSuccess: function onSuccess() {
            _s._loadAtom(mediaFileReader, offset + atomSize, parentAtomFullName, callbacks);
          },
          onError: callbacks.onError
        });
      }
    ...
    key: "_parseData",
    value: function _parseData(_d, tagsToRead) {
      var tags = {};
      tagsToRead = this._expandShortcutTags(tagsToRead);

      this._readAtom(tags, _d, 0, _d.getSize(), tagsToRead); // create shortcuts for most common _d.
      ...
      return {
        "type": "MP4",
        "ftyp": _d.getStringAt(8, 4),
        "version": _d.getLongAt(12, true),
        "tags": tags
      };
    }
  }, {
    key: "_readAtom",
    value: function _readAtom(tags, _d, offset, length, tagsToRead, parentAtomFullName, indent) {
      indent = indent === undefined ? "" : indent + "  ";
      var seek = offset;

      while (seek < offset + length) {
        var atomSize = _d.getLongAt(seek, true);

        if (atomSize == 0) {
          return;
        }

        var atomName = _d.getStringAt(seek + 4, 4); // console.log(seek, parentAtomFullName, atomName, atomSize);
        ...
        if (this._isContainerAtom(atomName)) {
          ...
          this._readAtom(tags, _d, seek + 8, atomSize - 8, tagsToRead, atomFullName, indent);
        ...
        if ((!tagsToRead || tagsToRead.indexOf(atomName) >= 0) && parentAtomFullName === "moov.udta.meta.ilst" && this._canReadAtom(atomName)) {
          tags[atomName] = this._readMetadataAtom(_d, seek);
        }
    ...
    value: function _readMetadataAtom(_d, offset) {
      // 16: name + size + "_d" + size (4 bytes each)
      var METADATA_HEADER = 16;
      var atomSize = _d.getLongAt(offset, true);
      var atomName = _d.getStringAt(offset + 4, 4);
      var klass = _d.getInteger24At(offset + METADATA_HEADER + 1, true);
      var type = TYPES[klass];
      var atomData;

      if (atomName == "trkn") {
        atomData = {
          "track": _d.getByteAt(offset + METADATA_HEADER + 11),
          "total": _d.getByteAt(offset + METADATA_HEADER + 13)
        };
      } else if (atomName == "disk") {
        atomData = {
          "disk": _d.getByteAt(offset + METADATA_HEADER + 11),
          "total": _d.getByteAt(offset + METADATA_HEADER + 13)
        };
      } else {
        ...
        switch (type) {
          case "text":
            atomData = _d.getStringWithCharsetAt(dataStart, dataLength, "utf-8").toString();
            break;

          case "uint8":
            atomData = _d.getShortAt(dataStart, false);
            break;
          ...
          case "uint":
            ...
            //
            var intReader = type == 'int' ? dataLength == 1 ? _d.getSByteAt : dataLength == 2 ? _d.getSShortAt : dataLength == 4 ? _d.getSLongAt : _d.getLongAt : dataLength == 1 ? _d.getByteAt : dataLength == 2 ? _d.getShortAt : _d.getLongAt; // $FlowFixMe - getByteAt doesn't receive a second argument

            atomData = intReader.call(_d, dataStart + (dataLength == 8 ? 4 : 0), true);
            break;
          ...
          case "png":
            atomData = {
              "format": "image/" + type,
              "_d": _d.getBytesAt(dataStart, dataLength)
            };
            break;
        }
      }
  ...
  _createClass(MediaTagReader, [{
    ...
    value: function read(callbacks) {
      ...
      this._mediaFileReader.init({
        onSuccess: function onSuccess() {
          _s._loadData(_s._mediaFileReader, {
            onSuccess: function onSuccess() {
              try {
                var tags = _s._parseData(_s._mediaFileReader, _s._tags);
              } catch (ex) {
                if (callbacks.onError) {
                  callbacks.onError({
                    "type": "parseData",
    ...
    /**
     * Parse the loaded _d to read the media tags.
     */
  ...
  _createClass(XhrFileReader, [{
    ...
    value: function _fetchSizeWithHeadRequest(callbacks) {
      ...
      this._makeXHRRequest("HEAD", null, {
        onSuccess: function onSuccess(xhr) {
          var contentLength = _s._parseContentLength(xhr);

          if (contentLength) {
            _s._size = contentLength;
            callbacks.onSuccess();
          } else {
            // Content-Length not provided by the server, fallback to
            // GET requests.
            _s._fetchSizeWithGetRequest(callbacks);
          }
        },
        onError: callbacks.onError
      });
    ...
    value: function _fetchSizeWithGetRequest(callbacks) {
      ...
      this._makeXHRRequest("GET", range, {
        onSuccess: function onSuccess(xhr) {
          var contentRange = _s._parseContentRange(xhr);

          var _d = _s._getXhrResponseContent(xhr);
          ...
          if (contentRange) {
            if (contentRange.instanceLength == null) {
              ...
              _s._fetchEntireFile(callbacks);

              return;
            }

            _s._size = contentRange.instanceLength;
          } else {
            // Range request not supported, we got the entire file
            _s._size = _d.length;
          }

          _s._fileData.addData(0, _d);
    ...
    value: function loadRange(range, callbacks) {
      ...
      if (_s._fileData.hasDataRange(range[0], Math.min(_s._size, range[1]))) {
        setTimeout(callbacks.onSuccess, 1);
        return;
      ...
      range[1] = Math.min(_s._size, range[1]);

      this._makeXHRRequest("GET", range, {
        onSuccess: function onSuccess(xhr) {
          var _d = _s._getXhrResponseContent(xhr);

          _s._fileData.addData(range[0], _d);
    ...
    value: function _isWebWorker() {
      return typeof WorkerGlobalScope !== 'undefined' && _s instanceof WorkerGlobalScope;
    }
  }, {
...
function () {
  ...
  _createClass(Reader, [{
    ...
    value: function read(callbacks) {
      ...
      fileReader.init({
        onSuccess: function onSuccess() {
          _s._getTagReader(fileReader, {
            onSuccess: function onSuccess(TagReader) {
              new TagReader(fileReader).setTagsToRead(_s._tagsToRead).read(callbacks);
            },
            onError: callbacks.onError
          });
        },
